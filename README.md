...
以 OKX 公開市場資料運作的加密資產 USDT 線性永續合約雙雷達。全市場 Universe 只接受 OKX `instCategory=1`，並對 24H USDT 成交額套用「上 200 萬、向下緩衝 50 萬」規則：新標的達 200 萬就納入，已納入標的跌破 150 萬才移除，避免在 200 萬附近反覆進出。股票型永續（`instCategory=3`）、其他非加密分類與未通過緩衝帶的合約都會在多週期 K 線請求前排除。系統只做分析，不接受 API Key、Secret 或 Passphrase，也沒有自動下單、Paper Trading 或 Live Trading 路徑。V3.4 延續既有 Price-first（價格優先）Trigger 與 Signal Episode（訊號生命週期）。正式 Trigger／卡片與「掃描快照中的新進場條件」分開：輔助 Market Context 不會翻轉方向；正式反向訊號、缺少或過期的執行價格、硬性行情異常、Spread／已知 Slippage／已知來回成本超標或 R:R 不足，則會保留原卡但封鎖新的進場。若 Order Book 偶發缺失而無法估算滑價／成本，只顯示提醒，不單獨禁止進場。

手機介面將 Entry Zone 統一顯示為「Entry（可進參考區間）」，並依「方向 → ⚡ 已觸發訊號 → 進場快照狀態 → 阻擋原因 → Entry／SL／TP／R:R → 進場前更新 → 輔助判讀」排列。進場快照只顯示「待進場確認、等待回踩、風控受阻、等待新訊號」，不把掃描結果寫成持續即時狀態；卡片與進場前更新都附資料時間。續走力道與 OI 預設收合，摘要只用白話顯示最近 1H 的 OI 增減、推測方向與「僅供輔助」；展開後才看依據、信心與 1H／2H／4H 比較。它不產生加分或一致度分數，也不加入正式方向、進場判定或目標計算。首頁顯示 15m／4H 有效的已觸發訊號數量；硬性阻擋原因與高週期衝突直接顯示在卡片上方。市場方向由每個具有連續完整 1H 收盤資料的市場，以 25 個收盤計算 24H RSI(24) 後作全市場等權平均，直接顯示真實數字與「強多／偏多／中性／偏空／強空」；原有 15m／4H RSI(14) 平均與樣本數仍保留，不拿 BTC RSI 冒充市場平均。通知與收藏改為收合，完整 OI 異動雷達移到「更多」。Funding（資金費率）、Spread（買賣價差）、Slippage（滑價）與 Order Book（訂單簿）仍收進詳細資料。單幣請求期間只使用輕量 CSS 掃描動畫，不載入 GIF、影片、Canvas 或大型外部資源。

每張已有正式 Entry／SL／TP 的訊號卡都提供複製交易計畫按鈕，包括待進場確認、等待回踩、風控受阻、等待新訊號與已達 TP／SL 的結果卡；未形成正式 Trigger 的觀察卡不顯示不存在的交易計畫。
...
V3.4 的核心原則是：**正式價格 Trigger 成立就保留卡片，但保留卡片不等於允許新進場**。價格／結構與 MA／MACD 負責建立 Trigger；掃描另外用「同向延續確認」白話描述最近完整收線的資金與量能。OI、Taker／CVD、Funding 與 Order Book 都只是輔助理解，不建立或取消 Trigger、不改方向、不給進場權限，也不延伸 TP／R:R；良好深度不會替候選加分，只有差的執行條件會扣分或觸發獨立硬門檻。掃描時價格位置、已收線重新確認與 Hard Gate 共同形成「待進場確認、等待回踩、等待新訊號、風控受阻」快照；Hard Gate 失敗時仍保留 Trigger 與原交易計畫，供既有部位管理與後續追蹤。
...
6. 整理 Market Context、變化趨勢與異常資料供詳細頁參考；資料不足就顯示不知道，不填假值，也不讓 OI／Taker／Funding／Order Book 決定正式方向或新進場。
7. 正式發布前再取得一次 bulk ticker。LONG 以最新 Ask、SHORT 以最新 Bid 作為進場幾何的可執行價格；Ticker 缺失、非有限值、早於本輪掃描或無法取得對應方向價格時一律 fail closed，顯示資料不可用而不沿用掃描起點舊價。
8. 最後依新鮮可執行價格相對 Entry／SL／TP 的位置與 Hard Gate 輸出本輪判定。軟性背景只提醒；反向正式訊號、硬性異常、Spread／Slippage／成本超標或 R:R 不足會封鎖新進場，但不刪除正式 Trigger。
9. SQLite 保存 Signal Episode、事件、MFE／MAE、TP／SL 先後與結果，再從真實完成樣本計算績效。
10. 每個雷達最多顯示 20 個訊號；不足就顯示 0 個，不為湊數放寬標準。
...
| 資料 | V3.4 用法 |
| --- | --- |
| Open Interest | 輔助延續描述；本身沒有多空方向，必須與價格變化組合推測新多／新空、平倉或回補，不參與正式方向、進場、排名或 TP／R:R |
| Taker Flow／CVD | 輔助延續描述；兩者同源，必須與價格成果一起看；流量很強但價格推不動視為可能吸收，不取得進場或目標決定權 |
| 核心 K 線量比 | 第三類延續證據；量比達 1.2 倍且價格同向才支持延續，放量但價格反向則是衝突 |
| Funding | 只顯示擁擠程度與警告，不直接判多空、不授予或封鎖進場，也不改目標 |
| Order Book | 只作執行背景；首張快照不當支撐壓力，跨掃描比較 persistence、撤單、補單、吸收與反向深度。好深度不加分，差 Spread／Slippage／成本可扣分或封鎖，但不改正式方向或目標 |
| BTC／全市場／高週期 | 辨識相對強弱、市場帶動、逆高週期與重複曝險；不替個別標的建立、取消或翻轉 Trigger，重大已收盤核心價格衝突可封鎖新進場 |
...
正式掃描仍是使用者手動啟動的市場快照。掃描當下會直接取得單一合約的歷史 OI，並把 OKX 的資料生成時間歸到其前一個已完成 5m K 收線點：15m 短線以 2／3 個端點建立 5／10 分鐘窗，4H 波段以 7／13 個端點建立 30／60 分鐘窗。因此卡片不必先等待訊號後累積 10 或 60 分鐘，便能顯示掃描當下的平均續走力道。OI 會同時公開最新值、前段端點均值、絕對增加／減少量與百分比；再配合同窗已收盤價格推定偏多或偏空，單次增加只作方向提示，未通過連續性門檻不會提高續走力道。5m／30m 快速窗用來提早辨識近期轉弱，10m／60m 基準窗決定主要分級；快速窗不能單獨把力道升到最高級。
...
- 突破追價距離以突破邊界／Entry Zone 計算；從最近防守點累積的整段推進只作警告，避免把剛越過邊界的新 Trigger 誤判為「等待新訊號」。
...
Trigger 是否存在與「掃描快照中的進場條件」分開顯示。下列是內部狀態及其前端文案：

- `ENTRY_READY` →「待進場確認」：掃描時可執行價格仍在 Entry Zone 或只順向偏離最多 0.15 ATR，且已收線確認與全部 Hard Gate 都通過；卡片仍只是快照，R:R 不足時會保留卡片但封鎖新進場。
- `WAIT_RETEST` →「等待回踩」：Trigger 仍存在，但已偏離 Entry Zone；等待回踩／重新確認，不追價。
- `MISSED_ENTRY` →「等待新訊號」：順向偏離超過 0.50 ATR，或生命週期已離開進場階段；原進場窗口關閉，保留故事追蹤但不建議在延伸位置追價。
- `HARD_GATE_BLOCKED` 等硬性狀態 →「風控受阻」：Trigger 與原計畫保留，但資料或風險門檻不允許沿用該快照進場。

價格若落到原 Entry Zone 的不利側，仍保留原 Trigger 作生命週期追蹤；但舊 Episode 或曾經 `ENTRY_READY` 的卡片，不能只因價格再次進入原 Entry Zone 就自動恢復可進。必須出現同一方向新的已收線回踩／收復確認，才可重新評估進場。等待期間即時 Stop 距離可能極小，直接用目前價格計算會產生失真的超大 R:R，因此頁面顯示「暫不適用」；這只修正位置判定與顯示，不改寫原 Entry／Stop／Target 或 V3.4 Trigger。

單幣掃描將「訊號生命週期」與「本次更新快照」分開顯示。所有有效正式卡先標記「⚡ 已觸發訊號」，再顯示待進場確認、等待回踩、風控受阻或等待新訊號；順向走遠不追價，不利側尚未碰到原始 SL 時則等待新的收線回踩／收復確認或接近失效。這些固定時間快照不會刪除原卡，也不會改寫原 Entry／SL／TP；若出現正式反向訊號，原方向新進場會暫停，舊計畫僅供既有部位管理。
...
後端先依正式進場權限、交易品質、資料新鮮度、剩餘 R:R、較低滑價與較高流動性選出最多 20 張卡；OI／Taker／Funding、Order Book 的方向解讀與續走結果不參與這個截斷，只有 Order Book 換算出的實際 Spread／Slippage／成本惡化能透過交易品質扣分或硬門檻影響結果。前端仍把內部 `ENTRY_READY` 的「待進場確認」卡排在前面，再比較交易品質、資料時間與其他執行條件；缺少品質分數的卡排在最後，曾經通過不會取得排序優先權。這不會刪卡或改寫方向；同一 Episode 也保留原 Entry／Stop／Target，不因刷新而漂移。

手機訊號卡使用交易終端式資訊層級：首列突出幣種、週期、方向，以及獨立的「⚡ 已觸發訊號」生命週期標記與「待進場確認／等待回踩／風控受阻／等待新訊號」快照狀態。下一層明確寫「進場快照｜不是即時報價」，再顯示 Hard Gate／反向正式訊號的阻擋原因及高週期衝突提示；接著直接列可進參考區間、SL、TP1 與 R:R 計畫及進場前更新按鈕。續走力道與 OI 移到其後的「輔助判讀」並預設收合。首屏不再攤開 OI／Taker／量能拆解、分數、採樣門檻、判定長句、TP2 說明、觸發時間與次要操作；完整技術依據收進「完整判定資料」，時間與操作收進「原因、時間與其他操作」。TradingView 與複製計畫維持次要操作。完整多週期、MA／MACD、Market Story 與安全資料預設收合；續走力道是軟確認，不代表勝率，也不改變 Trigger 或進場資格。

## 真實歷史績效

`data/radar_state.sqlite3` 保存每個訊號的版本、方向、週期、Trigger 類型、Market Participation、Execution Quality、MFE、MAE、TP1／SL 先後與 Final R。

新形成的交易計畫會使用已關閉 Signal Episode 做分層 MAE／MFE 自動學習：優先採用同幣種＋同週期＋同方向＋同 Trigger 樣本，樣本不足才逐層退到同幣種或同週期／方向群組。止損只採成功 Episode 的 MAE 第 80 百分位並加上影線／執行緩衝，避免失敗交易教系統無限放寬止損；止盈採全部有效完成 Episode 的 MFE 第 60／80 百分位，再與已收盤價格結構、真實波動、核心 K 線量比與 Timing 組合。OI、Taker／CVD、Funding 與 Order Book 不會拉遠止盈或提高 R:R。每一層至少需要 5～20 筆樣本；不足時維持結構、真實波動、ATR 與百分比保底。學習只作用於全新 Trigger 的新卡，既有 Episode 的 Entry／SL／TP 永遠不改寫。
...
- 正式發布前會重新取得 bulk ticker 並記錄發布價格狀態；LONG Ask／SHORT Bid 缺失、無效、過期或刷新失敗時 fail closed，不沿用掃描開頭的 Last Price 取得新進場資格。
...
- 首頁先顯示「已觸發訊號」，內部 `ENTRY_READY` 的待進場確認候選仍優先排列；市場概況保留，收藏與掃描通知預設收合，完整 OI 異動移到「更多」
- 首頁把「⚡ 已觸發訊號」與進場快照狀態分成兩層；阻擋原因醒目顯示，首頁數量計算有效已觸發訊號，不把執行環境分數當成進場許可
- 15m 與 4H 長線皆有全部、待進場確認、等待回踩、等待新訊號與接近觸發分頁
...
- V3.4 新計畫的 SL 採結構、ATR、近期真實波幅與影線分布的動態緩衝；TP 由已收盤價格結構、行情型態、真實波動、核心量比、Timing 與可用歷史結果生成，OI／Taker／Funding／Order Book 不延伸目標，近端小於 1.5R 的障礙只列途中觀察
- 真實歷史統計分頁
...
| `quote_volume_buffer_24h` | 500,000 | 從納入線向下延伸的保留緩衝：新標的達 2,000,000 就納入；已納入標的跌破 1,500,000 才移除；單幣／進場前更新只讀取可信會員狀態，不自行納入或繞過門檻 |
| `universe_max_spread_pct` | 1.00 | Universe 極端 Spread 提醒值 |
| `max_spread_pct` | 0.10 | 新進場 Spread 硬門檻；超過時保留卡片但封鎖本輪新進場 |
| `min_open_interest_usd` | 3,000,000 | OI Context 參考／舊設定相容，非硬門檻 |
| `minimum_rr` | 1.8 | 新進場最低 R:R；低於此值保留正式卡片但封鎖本輪新進場 |
| `execution_notional_usdt` | 1,000 | 公開深度滑價估算名目金額，不會下單 |
| `max_execution_cost_to_risk_pct` | 15 | 來回成交成本占風險的新進場硬門檻；超過時保留正式訊號但不放行 |
| `max_slippage_pct` | 0.15 | 方向性 Slippage 新進場硬門檻；超過時保留正式訊號但不放行 |
...
| `entry_ready_max_chase_atr` | 0.15 | 內部 `ENTRY_READY` 的順向偏離上限；前端顯示「待進場確認」 |
| `entry_missed_chase_atr` | 0.50 | 超過即關閉原進場窗口；前端顯示「等待新訊號」 |
...
成交量緩衝的會員清單會隨最新報告保存並在同一持久資料目錄內還原；但目前 `render.yaml` 沒有掛載 Render Persistent Disk。若免費 instance 重新部署或重啟後檔案系統被重建，會員歷史會安全地重置為非會員：標的必須重新達到 2,000,000 USDT 才能納入，不會因缺少歷史而誤用 1,500,000 保留線。
...
測試涵蓋 Price Trigger 與進場資格分離、OI／Taker-CVD／核心量比的同向延續輔助描述、OKX 歷史 OI 生成時間映射至已收線 5m K、最新端點對前段均值、SHORT 2／3 點的 5／10m 視窗、LONG 7／13 點的 30／60m 視窗、未來端點排除、衝突重複點拒收、缺棒不內插、SHORT／LONG 共用一次 OI 請求、raw OI 單位隔離、Taker／成交量單一尖峰不翻轉平均、缺價格不冒充吸收、舊 observer 不接手、新核心 generation 重啟、輔助延續分級不刪卡不翻向也不改進場、早期訊號在同一 Episode 升級且 Entry／SL／TP 不漂移、舊 Entry Zone 不自動重開、正式反向訊號暫停原方向新進場、TP／SL 結果卡的 5／24 小時期限與獨立已結束板塊、股票型／非加密合約在全市場及單一合約入口被排除、24H USDT 成交額 200 萬納入／150 萬移除的緩衝、單幣來源並行與短等待上限、結構＋波動分布止損、OI／Taker／Funding／Order Book 不改自動目標、正式發布 Ticker 刷新與 LONG Ask／SHORT Bid 進場幾何、執行價格缺失 fail closed、Spread／Slippage／成交成本／R:R／硬性異常綁定封鎖新進場但保留卡片、資料不知道不冒充最新、Market Context／DST 時段、價格接受、控制權轉移、Signal Episode 去重與永久失效、舊資料／亂序資料不回寫、15m／4H 隔離、三種掃描、部分掃描與獨立新鮮度、`CORE_PREVIEW` 唯讀、已觸發訊號與進場快照分層、待進場確認排序、醒目阻擋原因、Scan Lock、舊請求不可覆蓋新結果、STALE 快照保留、API 失敗降級、Web Push、PWA 與 API contract。
...
## 2026-09-09：日內段落與單幣價格／持倉交叉判讀

- 恢復獨立「幣種掃描」入口，與「進場前更新」分開。卡片上的掃描傳入原方向；不將多卡直接改成空卡、不重算原始 Entry／SL／TP。新反向主 Episode 僅由一般市場掃描建立。
- 市場掃描可接受獨立且時間更新的正式反向價格事件，不再單純因舊計畫尚未碰 SL 永久壓住反向 Trigger。舊計畫存成 `SUPERSEDED` 歷史，保留價位與事件；這不是實際平倉／止損，不記造假的 R。交易風控与位置判斷保持不變。
- 4H 已收線背景與內部最近四根15m段落分開描述；不要求4H翻空才承認15m回落。未收線不構成正式事件。
- 單幣結果及進場前更新新增最近15m／1H／4H價格、原始 OI 數量、主動買賣與區間差額。按同一最近完整5m資料截止點計算；最多等待一個5m資料桶，不用舊資料偽裝即時。
- OI沿用合約專用 history（張數優先、同單位幣數後備），不以 `oiUsd` 判增減倉；單幣5m OI請求擴到60點，全市場仍20點。CVD使用合約專用 `taker-volume-contract?unit=2`，一個最多60桶的有界請求；API失敗、缺桶、時間或K線成交額核對失敗均標資料暫缺。
- CVD顯示的是區間主動買入減主動賣出，不是App有未知重置起點的累積值。四小時窗口金額較大不等於力道較強；前後15m比較用同期成交差額比例。這些資料不創造、取消、加分或翻轉核心訊號。
- 本次沒有新增常駐收集、排程、付費服務或下單接口。自動化合成資料測試驗證工程行為，不能等同XPL當時漏報已重現或交易勝率已驗證。

API依據：OKX官方V5文件的 Get contract open interest history 與 Get contract taker volume（2026-09-09查核）。


## 2026-09-09：精簡介面與止盈止損管理參考

- 主卡統一為「進場狀態 → 兩個主要理由 → Entry／SL／TP1／TP2／R:R → 查看數據」。完整理由、多週期、持倉與執行細節移到獨立資料視窗；不刪原數據，也不改進場資格。
- 「價格・OI・CVD 多週期判讀」改為白話「資金與買賣力道」摘要和三個小標籤。打開資料視窗後切換 15m／1H／4H；顯示相同起訖時間的價格、持倉與主動買賣。OI 使用藍色中性資訊，不把增倉直接塗成做多。
- 全站色彩、卡片間距、手機寬度與資料視窗統一；多／買方為綠，空／賣方為紅，止盈綠、止損紅、風險提醒黃，並保留文字標籤。資料視窗支援返回、Escape 和焦點還原，不關掉底下的單幣或進場前更新畫面。
- 單幣及進場前更新新增唯讀 `EXIT_REVIEW_V1` 管理參考。結合原始結構／波動計畫、最新同窗價格、原始持倉、經核對的主動成交，提示按原目標觀察、反推風險或近端結構分批參考；不假定已持倉、不送單、不更改原 Entry／SL／TP。
- 短線保護止損參考只在有至少 1R 順向空間時，使用已收線15m高低與5m真實波幅緩衝，按 tick 向外取整。必須比原SL更靠近、位於現價安全一側；長線不得用15m結構移動SL。沒有合約 tick、資料過期、不同幣或已終止時不生成管理價格。若使用者已採用更緊止損，本參考不得用來放寬它。
- TP1／TP2 原始計算不變；OI／CVD 不機械式放大止盈或止損。管理參考是新增的保守規則，未經實盤績效校準，不代表收益保證或實際掛單。
- 不新增請求、背景排程、付費服務、自動交易或部署設定。合成資料與瀏覽器驗收不代表線上實際行情已驗證。
