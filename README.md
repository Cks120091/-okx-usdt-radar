...
手機訊號卡使用交易終端式資訊層級：首列只突出幣種、週期、方向與交易品質，下一層用較大文字回答「現在能否進場」，緊接著顯示訊號後平均走向摘要，最後才列 Entry、SL、TP 與 R:R 四格計畫。續走摘要不再於頁首重複顯示；首屏只顯示採樣進度、最高等級門檻與目前 `OI＋加成` 組合，快速／基準窗及 `OI｜必要`、`Taker／CVD｜加成`、`成交量｜加成` 三列文字狀態收進「查看採樣與三項平均依據」。紅色反證仍留在首屏，狀態也不只靠顏色。完整多週期、MA／MACD、Market Story 與安全資料預設收合。`證據一致` 只代表資金與成交證據較一致，不代表勝率。
