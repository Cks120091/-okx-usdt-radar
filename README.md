...
V3.4 的核心原則是：**正式價格 Trigger 成立就保留卡片，但保留卡片不等於允許新進場**。價格／結構與 MA／MACD 負責建立 Trigger；掃描另外用「同向延續確認」白話描述最近完整收線的資金與量能。OI、Taker／CVD、Funding 與 Order Book 都只是輔助理解，不建立或取消 Trigger、不改方向、不給進場權限，也不延伸 TP／R:R；良好深度不會替候選加分，只有差的執行條件會扣分或觸發獨立硬門檻。現價位置、已收線重新確認與 Hard Gate 共同決定目前可進、等回踩、已錯過或封鎖；Hard Gate 失敗時仍保留 Trigger 與原交易計畫，供既有部位管理與後續追蹤。
...
OI 快速清洗、Funding 極端或 Order Book 變化會列為輔助背景，不取消正式 Trigger，也不把訊號翻成反向。瞬間插針等硬性行情異常，以及 Spread／Slippage／來回成本超標、執行價格缺失或過期、R:R 不足，會保留原 Trigger 與卡片但移出目前可進區，直到重新掃描並通過安全檢查。
...
- `ENTRY_READY`：最新可執行價格仍在 Entry Zone 或只順向偏離最多 0.15 ATR，且已收線確認與全部 Hard Gate 都通過；R:R 不足時會保留卡片但封鎖新進場。
...
單幣掃描將「訊號生命週期」與「目前新進場資格」分開顯示。曾列入「可進」只會留下歷史標記，不會固定算進目前可進分頁或數量；順向走遠會顯示目前不要追價，不利側尚未碰到原始 SL 時則顯示等待新的收線回踩／收復確認或接近失效。這些即時位置提醒不會刪除原卡，也不會改寫原 Entry／SL／TP；若出現正式反向訊號，原方向新進場會暫停，舊計畫僅供既有部位管理。
...
後端先依正式進場權限、交易品質、資料新鮮度、剩餘 R:R、較低滑價與較高流動性選出最多 20 張卡；OI／Taker／Funding、Order Book 的方向解讀與續走結果不參與這個截斷，只有 Order Book 換算出的實際 Spread／Slippage／成本惡化能透過交易品質扣分或硬門檻影響結果。前端先把本輪真正通過所有條件的目前可進卡排在前面，再比較交易品質、資料時間與其他執行條件；缺少品質分數的卡排在最後，曾經可進不會取得排序優先權。這不會刪卡或改寫方向；同一 Episode 也保留原 Entry／Stop／Target，不因刷新而漂移。

手機訊號卡使用交易終端式資訊層級：首列突出幣種、週期、方向與即時「目前可進／先不要進／等回踩／已錯過」狀態，下一層用較大文字回答「現在能否進場」，並優先顯示 Hard Gate／反向正式訊號的阻擋原因及高週期衝突提示；接著直接列 Entry、SL、TP1 與 R:R 四格計畫及進場前更新按鈕。續走力道與 OI 移到其後的「輔助判讀」並預設收合。首屏不再攤開 OI／Taker／量能拆解、分數、採樣門檻、判定長句、TP2 說明、觸發時間與次要操作；完整技術依據收進「完整判定資料」，時間與操作收進「原因、時間與其他操作」。TradingView 與複製計畫維持次要操作。完整多週期、MA／MACD、Market Story 與安全資料預設收合；續走力道是軟確認，不代表勝率，也不改變 Trigger 或進場資格。
...
- 首頁先顯示「目前最值得看」，只有本輪真正通過價格位置、已收線確認與 Hard Gate 的目前可進候選會優先排列；市場概況保留，收藏與掃描通知預設收合，完整 OI 異動移到「更多」
- 首頁候選先區分「目前可進」與「觀察／受阻」；阻擋原因醒目顯示，曾經可進只留在生命週期歷史，不計入目前可進數量，執行環境分數不會被當成進場許可
- 15m 與 4H 長線皆有全部、目前可進、等待回踩、已錯過與接近觸發分頁
