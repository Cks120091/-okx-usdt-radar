...
每張已有正式 Entry／SL／TP 的訊號卡都提供複製交易計畫按鈕，包括早期、完整確認、等待回踩、必要條件未成立、等待新訊號與已達 TP／SL 的結果卡；未形成正式 Trigger 的觀察卡不顯示不存在的交易計畫。
...
V3.4 的核心原則是：**正式價格 Trigger 成立就保留卡片，但保留卡片不等於允許新進場**。價格／結構與 MA／MACD 負責建立 Trigger；掃描另外用「同向延續確認」白話描述最近完整收線的資金與量能。OI、Taker／CVD、Funding 與 Order Book 都只是輔助理解，不建立或取消 Trigger、不改方向、不給進場權限，也不延伸 TP／R:R。執行風險可降低交易品質排序並顯示建議，但不否決新進場。掃描時只有訊號生命週期、正式反向訊號、必要資料與價格位置形成「掃描條件通過、等待回踩、等待新訊號、必要條件未成立」快照；風險建議另外顯示，不改寫 Trigger 與原交易計畫。
...
| 4. Risk Review（風險檢視） | 流動性、行情異常、Spread、Slippage、成本、R:R 與 Stop 寬度如何 | 一律作醒目建議，不替使用者禁止進場；正式反向訊號、計畫失效與必要資料不屬於風險建議，仍是必要條件 |

Market Context、OI、Taker、CVD、Funding 與 Order Book 仍會保存並放在詳細資料中，協助理解行情。OI 必須配合同窗價格才能白話推測「新多資金進場／新空資金進場／空單回補／多單平倉」，不能拆成不存在的「多單 OI／空單 OI」。OI、Taker／CVD 與核心 K 線量比只用來產生獨立的同向延續描述；Funding 與 Order Book 顯示擁擠或執行背景。這些資料不設定正式方向、不授予或加強進場，也不放大 TP／R:R；良好 Order Book 至多為中性，不會改善排序，差的 Spread／Slippage／成本可降低執行品質並顯示建議，但不封鎖新進場。BTC／全市場與已收盤高週期價格衝突另外顯示；正式反向價格訊號可以使原方向新進場條件不成立，但仍不能把原 Trigger 靜默翻向。資料缺失就顯示不知道，不用舊值、0 或中性假設補算。
...
8. 最後依新鮮可執行價格相對 Entry／SL／TP 的位置與必要條件輸出本輪判定。正式反向訊號、計畫失效、必要資料缺失或 Entry Window 未開放會使新進場條件不成立；行情異常、Spread／Slippage／成本、流動性與 R:R 只顯示建議。
...
| Order Book | 只作執行背景；首張快照不當支撐壓力，跨掃描比較 persistence、撤單、補單、吸收與反向深度。好深度不加分，差 Spread／Slippage／成本可扣交易品質並顯示建議，但不改正式方向、目標或進場權限 |
| BTC／全市場／高週期 | 辨識相對強弱、市場帶動、逆高週期與重複曝險；不替個別標的建立、取消或翻轉 Trigger。背景衝突只提醒；只有另一個正式反向 Trigger 會暫停原方向新進場 |
...
收線 bucket 重複衝突、缺格、間隔中斷或 K 線邊界不符時，該領域維持 `UNKNOWN`；只有整個基準比較窗未建立時，卡片才顯示「資料不足」。基準窗已完整、但 OI／量能沒有同向支持時顯示「偏弱」，已有一項支持顯示「中等」，避免把「力道不強」錯叫成「沒有資料」。不補 0，也不拿零散快照冒充平均。Funding、深度與 Order Book 的變化仍只作 Context／風險提醒。Market Context 另外整理 Regime（行情型態）、Phase（階段）、Volatility（波動）、BTC／市場帶動與三大交易時段。所有同向延續結果都只提供資訊，不會取消價格 Trigger、授予或否決進場資格、刪除卡片、改變方向、生成反向正式訊號、改動必要條件、參與排名，或改寫 Entry／SL／TP／R:R 與生命週期。進場權限仍由新鮮價格位置、已收線價格確認、正式反向訊號、計畫有效性與必要資料決定。

續走力道不產生任何分數，也不參與訊號評分；真正的 Win Rate 仍只由已完成 Signal Episode 的 TP／SL 與 Final R 樣本計算。

OI 快速清洗、Funding 極端或 Order Book 變化會列為輔助背景，不取消正式 Trigger，也不把訊號翻成反向。瞬間插針等行情異常，以及 Spread／Slippage／來回成本超標、流動性或 R:R 不理想，會保留原 Trigger 並標示「風險建議｜不阻止進場」。執行價格缺失或過期則屬必要資料問題，會顯示「必要條件未成立」。

## Execution Quality 與 Trigger 分離

入場位置、結構 R:R、Stop 距離、Spread、深度、估算滑價與來回成本組成 `execution_quality`。它不回答「價格 Trigger 是否存在」或決定多空方向；正式 Trigger 可以繼續顯示，超標數值只影響品質說明／排序並列為風險建議，不否決本輪的新進場權限。Order Book 偶發缺失時，滑價與成本顯示「未估算」提醒，不沿用舊值、不補 0，也不單獨禁止進場；最新 Bid／Ask、價格位置與其他必要條件仍照常核對。

- 正式發布前再次更新 Ticker；LONG 使用最新 Ask、SHORT 使用最新 Bid 計算進場幾何。缺少、無效或早於本輪掃描的發布價格一律標為 `DATA_UNAVAILABLE`，不拿掃描起點舊價放行。
- Spread／已知 Slippage／已知來回成本超標、流動性不足、R:R／Stop 距離不理想與行情異常都只作醒目風險建議，不設定 `new_entry_allowed=false`，也不刪除或翻轉正式 Trigger。
- 突破追價距離以突破邊界／Entry Zone 計算；從最近防守點累積的整段推進只作警告，避免把剛越過邊界的新 Trigger 誤判為「等待新訊號」。
- OI 偏低、5m 輔助資料不一致或 Funding 擁擠只供參考，不取消有效核心 Trigger、不改進場權限，也不能改善候選排名或目標。Order Book 的方向解讀同樣只供參考；實際 Spread／Slippage／成本惡化可降低執行品質並提示風險，但不封鎖新進場。
...
- `ENTRY_READY` →「掃描條件通過」：掃描時可執行價格仍在 Entry Zone 或只順向偏離最多 0.15 ATR，且訊號、收線確認與必要資料都成立；它仍只是固定時間快照，下單前需更新。風險建議不會把它改成不可進。
...
- `HARD_GATE_BLOCKED`／`DATA_UNAVAILABLE` 等必要狀態 →「必要條件未成立」：Trigger 與原計畫保留，但正式反向訊號、計畫失效或必要資料問題不允許沿用該快照進場；一般風險數值不使用這個狀態。
...
單幣掃描將「訊號生命週期」與「本次更新快照」分開顯示。所有有效正式卡先標記「⚡ 訊號觸發」及早期／完整確認階段，再顯示掃描條件通過、等待回踩、必要條件未成立或等待新訊號；風險建議另以醒目提示呈現。順向走遠不追價，不利側尚未碰到原始 SL 時則等待新的收線回踩／收復確認或接近失效。這些固定時間快照不會刪除原卡，也不會改寫原 Entry／SL／TP。
...
後端先依正式進場權限、交易品質、資料新鮮度、剩餘 R:R、較低滑價與較高流動性選出最多 20 張卡；OI／Taker／Funding、Order Book 的方向解讀與續走結果不參與這個截斷，Order Book 換算出的實際 Spread／Slippage／成本惡化只透過交易品質排序與建議呈現。前端仍依相同執行品質順序排列，但不再建立 `ENTRY_READY` 專屬分頁或顯示「待進場確認」。這不會刪卡或改寫方向；同一 Episode 也保留原 Entry／Stop／Target，不因刷新而漂移。

手機訊號卡使用交易終端式資訊層級：首列突出幣種、週期、方向，以及獨立的「⚡ 訊號觸發」與「早期訊號／完整確認／回踩再發動」階段標記。下一層顯示固定時間的進場快照、風險建議、真正必要條件及高週期衝突提示；接著列可進參考區間、SL、TP1 與 R:R 計畫及進場前更新按鈕。續走力道與 OI 移到其後的「輔助判讀」並預設收合；它是軟確認，不代表勝率，也不改變 Trigger 或進場資格。
...
- 首頁先顯示「訊號觸發」；早期與完整確認卡統一排列，階段直接標在卡片頂端
...
| `max_spread_pct` | 0.10 | Spread 風險建議線；超過時保留卡片與進場權限並醒目提示 |
| `min_open_interest_usd` | 3,000,000 | OI Context 參考／舊設定相容，非硬門檻 |
| `minimum_rr` | 1.8 | R:R 風險建議線；低於此值保留正式卡片與進場權限並醒目提示 |
| `execution_notional_usdt` | 1,000 | 公開深度滑價估算名目金額，不會下單 |
| `max_execution_cost_to_risk_pct` | 15 | 來回成交成本占風險的建議線；超過時只顯示風險建議 |
| `max_slippage_pct` | 0.15 | 方向性 Slippage 建議線；超過時只顯示風險建議 |
...
| `entry_ready_max_chase_atr` | 0.15 | 內部 `ENTRY_READY` 的順向偏離上限；前端顯示「掃描條件通過」 |
...
- OI沿用合約專用 history（張數優先、同單位幣數後備），不以 `oiUsd` 判增減倉；單幣5m OI請求擴到60點，全市場仍20點。CVD使用合約專用 `taker-volume-contract?unit=2`，一個最多60桶的有界請求。全市場深度候選、單幣掃描及進場前更新都會把合約 Taker Buy／Sell 接到相同的完整5m OI／價格／量能窗口；只有幣種、USDT單位、精確桶時間及K線成交額（容差2%）全部吻合才投 `TAKER_CVD` 一票。API失敗、缺桶、衝突重複或核對失敗一律標資料不足，不用單次快照代替。
- CVD顯示的是區間主動買入減主動賣出，不是App有未知重置起點的累積值。四小時窗口金額較大不等於力道較強；前後15m比較用同期成交差額比例。這些資料不創造、取消、加分或翻轉核心訊號。
- 資金一致度把最近4小時拆成「最近15m、前一段15m、再前30m、前3H」四個非重疊區間（不代表統計獨立）。交叉判讀的試驗 OI 門檻分別為 ±0.1%、±0.1%、±0.2%、±0.5%，嚴格超過才計入；不是經回測校準的有效門檻，也不是自適應基準。增倉支持另外要求至少 2/3 根 5m OI 增加，且任一根的絕對變動不能占區間絕對變動總和 80% 以上。缺資料不補值；UI 顯示完整段數、共同快照時間、各段時間與持續性。原 15m／1H／4H 總覽仍沿用原始描述門檻。多空解讀是推測，不是實際開倉方向或勝率；不改 Trigger、Entry、SL、TP 或進場許可。此完整判讀僅接在單幣掃描及進場前更新，尚未擴充到全市場掃描。
...
## 2026-09-23：初篩與大盤參考修正

- 15m 大盤統計獨立使用本輪已取得核心資料的完整 Universe，不再只統計 MACD／MA 初篩候選。以 MA5/10/20 排列與 MACD 雙線同向描述市場方向；24H RSI 仍由各幣完整連續 25 根 1H 收盤計算後等權平均。這些背景觀察不建立交易訊號。
- 初篩保留交叉前形成中、當根交叉與交叉後同向延續。已有有效 Episode 的幣仍送核心重新檢查，不因未再交叉而漏掉管理更新；最終 Trigger 與進場資格仍由原核心判定。
- 大盤共振欄位不再被群體曝險資料覆蓋，公開卡片資料明確傳送共振狀態。缺大盤資料顯示未知，不當成中性加分。
- 沒有候選不再直接當成資料故障。修正附有市場樣本、延續初篩、Episode 更新與共振公開資料回歸測試。
