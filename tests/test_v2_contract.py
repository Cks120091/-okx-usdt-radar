...
class V33ContractTests(unittest.TestCase):
    ...
    def test_mobile_ui_triggers_real_scan_and_uses_chinese_lifecycle(self):
        ...
        self.assertIn("USDT PERPETUAL · MARKET INTELLIGENCE", html)
        ...
        self.assertIn("`${displaySymbol(normalized)} · 幣種掃描`", html)
        ...
        self.assertIn("okx-radar-shell-v3.3-auto-instrument-scan-1", service_worker)
        ...
        self.assertIn("④ 什麼情況代表舊計畫失效", html)
        self.assertIn("⑤ 方向現在怎麼看", html)
        ...
        self.assertIn("失效的是這一套「理想價格、SL、TP」交易計畫", html)
        self.assertIn("原始${direction}方向現在改為「等待重新判定」", html)
        self.assertIn("新的 Trigger／REENTRY", html)
        ...
        self.assertIn("function openInstrument(instId,{scanNow=true}={})", html)
        self.assertIn("if(scanNow)scanInstrument()", html)
        self.assertIn("function instrumentButton(instId,label='幣種掃描')", html)
        self.assertIn("${instrumentButton(item.inst_id)}", html)
        self.assertIn("${instrumentButton(instId)}", html)
        ...
        self.assertIn("這個「單幣掃描判定」在做什麼？", html)
        self.assertIn("① 原始訊號代表什麼", html)
        self.assertIn("② 現在為什麼是這個結果", html)
        self.assertIn("③ 你現在該怎麼做", html)
        self.assertIn("④ 什麼情況代表舊計畫失效", html)
        ...
        self.assertIn("訊號準備度不是進場許可", html)
