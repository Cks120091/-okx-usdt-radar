...
class V33ContractTests(unittest.TestCase):
    ...
    def test_mobile_ui_triggers_real_scan_and_uses_chinese_lifecycle(self):
        ...
        self.assertIn("grid-template-rows:minmax(0,1fr) auto auto", html)
        self.assertIn(".shell{grid-row:1;min-height:0", html)
        self.assertIn(".primary-nav{grid-row:3;position:relative", html)
        self.assertIn("--primary-nav-safe-bottom", html)
        self.assertIn(".action-wrap{display:none;grid-row:2;position:relative", html)
        self.assertIn("$('.shell').scrollTo", html)
